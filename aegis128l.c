				  ...
				  const u8 *iv)
{
	...
	for (i = 0; i < 8; i++)
	{
		printk("state->blocks_init %x \n",state->blocks[i].bytes[0]);
	}
}
...
for (i = 0; i < 8; i++)
	{
		printk("state->blocks_process_ad %x \n",state->blocks[i].bytes[0]);
	}
}
				   ...
				   const struct aegis128l_ops *ops)
{
	...
	printk("iv in aegis128l %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x \n",req->iv[0],req->iv[1],req->iv[2],req->iv[3],req->iv[4],req->iv[5],req->iv[6],req->iv[7],req->iv[8],req->iv[9],req->iv[10],req->iv[11],req->iv[12],req->iv[13],req->iv[14],req->iv[15]);
...
printk("hallo aegis128l encrypt");
...
static int crypto_aegis128l_decrypt(struct aead_request *req)
{
	...
	crypto_aegis128l_crypt(req, &tag, cryptlen, &ops);
	printk("tag.bytes %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x \n", tag.bytes[0], tag.bytes[1],tag.bytes[2],tag.bytes[3],tag.bytes[4],tag.bytes[5],tag.bytes[6],tag.bytes[7],tag.bytes[8],tag.bytes[9],tag.bytes[10],tag.bytes[11],tag.bytes[12],tag.bytes[13],tag.bytes[14],tag.bytes[15]);
printk("zero %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x \n", zeros[0], zeros[1],zeros[2],zeros[3],zeros[4],zeros[5],zeros[6],zeros[7],zeros[8],zeros[9],zeros[10],zeros[11],zeros[12],zeros[13],zeros[14],zeros[15]);
