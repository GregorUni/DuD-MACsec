...
make_info() {
  ...
  tc qdisc > $INFO_FILE
	...
	ip link show enx18d6c70c0a7e>> $INFO_FILE
...
#frame sizes to test-> 60 128 256 512 1024 1400 1514
	...
	if [[ $5 == mwe ]]; then #case macsec with aes(gcm) without encryption 
		...
		mtu_config_for_iperf3 $1 $2 1550 $DEST_IP
                ...
                #sudo ip link set dev eno1 mtu 60
                #eva_iperf $1 $2 60 $DEST_IP
		mtu_config_for_iperf3 $1 $2 164 $REMOTE_IP
		mtu_config_for_iperf3 $1 $2 292 $REMOTE_IP
		mtu_config_for_iperf3 $1 $2 548 $REMOTE_IP
		mtu_config_for_iperf3 $1 $2 1060 $REMOTE_IP
		mtu_config_for_iperf3 $1 $2 1436 $REMOTE_IP
		mtu_config_for_iperf3 $1 $2 1550 $REMOTE_IP
